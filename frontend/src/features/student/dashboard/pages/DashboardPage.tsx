...
import StudentStats from '@/app/components/StudentStats';
  ...
  useEffect(() => {
    ...
    const fetchMentors = async () => {
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || 'http://localhost:5000') + ''}/api/faculty/teaching-map?year=${year}&section=${section}&branch=${branch}`);
        const data = await res.json();
